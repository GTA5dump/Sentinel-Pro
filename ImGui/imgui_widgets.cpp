...
bool ImGui::TabButton(const char* label, bool selected, const ImVec2& size_arg)
{
    ...
    // Render
    RenderNavHighlight(bb, id);
    RenderFrame(bb.Min, bb.Max, selected ? ImGui::GetColorU32(ImGui::ColorConvertU32ToFloat4(ImColor(99, 66, 245, 255))) : ImGui::GetColorU32(ImGui::ColorConvertU32ToFloat4(ImColor(60, 60, 60, 255))), true, style.FrameRounding);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImVec2 label_size = ImVec2(CalcTextSize(label, NULL, true).x + 8.f, CalcTextSize(label, NULL, true).y);
    const ImRect frame_bb(ImVec2(window->DC.CursorPos.x + label_size.x, window->DC.CursorPos.y), ImVec2(window->DC.CursorPos.x + label_size.x, window->DC.CursorPos.y) + ImVec2(w, label_size.y + style.FramePadding.y * 0.5f));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f));
    ...
    // Draw frame
    const ImU32 frame_col = IM_COL32(60, 60, 60, 255);
    RenderNavHighlight(frame_bb, id);
    RenderFrame(frame_bb.Min, frame_bb.Max, frame_col, true, 12.25f);
    ...
    if (grab_bb.Max.x > grab_bb.Min.x)
        window->DrawList->AddRectFilled(grab_bb.Min, grab_bb.Max, GetColorU32(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab), 12.25f);
    // Display value using user-provided display format so user can add prefix/suffix/decorations to the value.
    char value_buf[64];
    const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, p_data, format);
    if (g.LogEnabled)
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end)
{
    ...
    if (display_frame)
    {
        ...
        const ImU32 bg_col = GetColorU32(ImGui::ColorConvertU32ToFloat4(ImColor(41, 41, 41, 0)));
